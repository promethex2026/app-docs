...
flowchart TB
    subgraph Factory ["PredictionFactory (TransparentUpgradeableProxy)"]
        direction LR
        FA["Access Control<br/>CREATOR_ROLE · UPGRADER_ROLE"]
        TW["Token Whitelist<br/>(USDC, …)"]
    end
    ...
    subgraph Market ["Prediction Market (Clone)"]
        direction TB
        APMM["APMM Engine<br/>deposit · withdraw<br/>addLiquidity · removeLiquidity"]
        LP["ERC-20 LP Token<br/>(Prediction is the LP)"]
        OPT["Option Tokens<br/>(ERC-20 per outcome)"]
    end
    ...
    RM -- "UMA path" --> UMA["UMA OptimisticOracleV3<br/>assertion → liveness → callback"]
    RM -- "Multisig path" --> MS["MultisigResolution<br/>N-of-M vote (planned)"]
...
Where \(r_i\) is the reserve of option \(i\), \(w_i\) is its weight, and \(k\) is the invariant constant. The instantaneous price of option \(i\) is:

$$
\text{Price}_i = w_i \cdot \frac{\sum_j r_j}{r_i}
$$
